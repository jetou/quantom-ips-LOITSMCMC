...
def _get_grids_dir():
    """Resolve grids directory: env var > cwd-relative > package-relative."""
    cand = os.environ.get("QUANTOM_GRIDS_DIR")
    if cand and os.path.isdir(cand):
        return cand
    # Try relative to cwd (user typically runs from project root)
    cand = os.path.join(os.getcwd(), "examples", "submission_scripts", "grids")
    if os.path.isdir(cand):
        return cand
    # Try relative to this file (repo layout: src/quantom_ips/envs/theories/)
    _base = os.path.dirname(os.path.abspath(__file__))
    for _ in range(4):
        _base = os.path.dirname(_base)
    cand = os.path.join(_base, "examples", "submission_scripts", "grids")
    if os.path.isdir(cand):
        return cand
    # Fallback: cwd-relative (create if needed)
    return os.path.join(os.getcwd(), "examples", "submission_scripts", "grids")
...
class SIDIS:
    ...
    def __init__(
        ...
        # ---- Cycle consistency settings ----
        self.enable_cc = True              # allow turning off without editing code
        self.cc_qt_min = 0
        self.cc_qt_max = 1.0
        self.cc_nqt    = 70                # number of qT points for CC
        self.cc_bmask_threshold = 2.0  # usually bmax=0.8
        self.cc_std_bt = 1.0               # scaling in loss
        self.cc_grid_dir = _get_grids_dir()
        self.cc_grid_tag = f"qtmin={self.cc_qt_min:g}_qtmax={self.cc_qt_max:g}_nqt={self.cc_nqt}"


        self.qt_cc = None
        self.bTqT_grid_cc = None
        self._cc_bmask = (self.bt > self.cc_bmask_threshold)  # CPU tensor for now
        self._cc_n_bmasked = int(self._cc_bmask.sum().item())

        self.cc_use_log_qt = False          # logspace qT grid
        if self.cc_use_log_qt:
            self.qt_cc = torch.logspace(np.log10(self.cc_qt_min), np.log10(self.cc_qt_max), self.cc_nqt)
        else:
            self.qt_cc = torch.linspace(self.cc_qt_min, self.cc_qt_max, self.cc_nqt)
    ...
    def build_bTqT_grid_CC(self):
        """
        CYCLE CONSISTENCY GRID GENERATION
        Generates a precomputed Fourier Transform matrix specifically for Cycle Consistency checks.
        It maps the bT space to a denser or different qT (transverse momentum) grid 
        defined by self.qt_cc using the Bessel J0 kernel.
        """
        checkdir(self.cc_grid_dir)
        filename = f"bTqT_grid_CC_{self.cc_grid_tag}_bTres={self.bTres}_ninterp={self.n_interp}.pt"

        
        # Numerical integration weights for the bT evaluation grid
        dbT = torch.diff(self.bt_eval)
        w_bT = torch.empty_like(self.bt_eval)
        w_bT[1:-1] = (dbT[:-1] + dbT[1:]) / 2
        w_bT[0], w_bT[-1] = dbT[0] / 2, dbT[-1] / 2

        grid_dir = self.cc_grid_dir
        
        if os.path.exists(f'{grid_dir}/{filename}'):
            self.bTqT_grid_cc = torch.load(f'{grid_dir}/{filename}', weights_only=True)
            print(f'CC Grid loaded: {filename}')
        else:
            t0 = time.time()
            # Projection matrix from model nodes to dense evaluation grid
            delta_bT = build_delta(self.bt, self.bt_eval, n=self.n_interp) 

            # Bessel kernel J0(qT * bT) computed via outer product for all combinations
            J = torch.special.bessel_j0(torch.outer(self.qt_cc, self.bt_eval))
            
            # Weighted kernel matrix for integration
            G = J * (self.bt_eval * w_bT)
            
            # Final matrix: contracts interpolation with the Bessel integral
            # Resulting shape: (n_bT_nodes, n_qT_cc)
            grid = torch.einsum('be,qe->bq', delta_bT, G)
            
            torch.save(grid, f'{grid_dir}/{filename}')
            self.bTqT_grid_cc = grid
            print(f'CC Grid generated in {time.time()-t0:.2f}s')


    def direct_inverse_transform(self, data, grid):
        """
        Performs a pseudo-inverse Hankel transform.
        First transforms the TMD PDF from bT space to k_T space using the CC grid, 
        then transforms it back to bT space to verify consistency.
        
        Args:
            data: Input densities from the generator.
            grid: Kinematic grid (x, Q2, z, etc).
        Returns:
            The reconstructed TMD PDF in bT space.
        """
        # 1. Calculate evolved TMD PDF in bT space (Batch, Flavor, x, Q, b)
        PDF_tmd = self.get_tmd_pdf(data, grid) 

        # 2. Forward Transform: bT -> k_T (qT)
        # Using the cycle consistency grid
        if self.bTqT_grid_cc is None:
            self.build_bTqT_grid_CC()
            
        bTqT_mat_cc = self.bTqT_grid_cc.to(data.device)
        # Contract over the 'b' dimension to produce qT distribution
        PDF_tmd_kt = torch.einsum('bq,BfxQb->BfxQq', bTqT_mat_cc, PDF_tmd)

        # 3. Inverse Transform: k_T -> bT
        # Generate the inverse kernel matrix on the fly
        qtd_1 = self.qt_cc.to(data.device)
        bt_nodes = self.bt.to(data.device)
        
        # Create meshgrid for the inverse kernel
        bt_mesh, qt_mesh = torch.meshgrid(bt_nodes, qtd_1, indexing='ij') 
        inv_bessel = torch.special.bessel_j0(qt_mesh * bt_mesh)
        
        # Matrix includes the 2*pi and qT factor for the Hankel inverse integration measure
        inv_matrix = torch.einsum('q,bq->bq', qtd_1, inv_bessel) * 2 * np.pi
        
        # Contract inverse matrix with the k_T space PDF
        integrand_ = torch.einsum('bq,BfxQq->BfxQbq', inv_matrix, PDF_tmd_kt)
        
        # Integrate over qT using the trapezoidal rule
        return torch.trapz(integrand_, qtd_1, dim=-1)


    def get_loss(self, data, grid):
        """
        Calculates a regularization loss based on Cycle Consistency.
        It compares the original model TMD PDF with the one obtained by 
        transforming to k_T space and back to bT space.
        
        Args:
            data: Input parameters for the density model.
            grid: Kinematic points.
        Returns:
            A scalar regularization penalty (log-likelihood style).
        """
        # Mask to focus the loss on the non-perturbative region (bT > 0.8)
        mask_bmax = self._cc_bmask.to(device=data.device, dtype=data.dtype)  # (nbt,)
        
        # Original TMD PDF from the model
        PDF_tmd = self.get_tmd_pdf(data, grid)
        # Flatten and mask for comparison
        pix_bt_flat = (PDF_tmd * mask_bmax).flatten(start_dim=1)    
                
        # Reconstructed TMD PDF via pseudo-inverse transform
        inverse_bt_pix = self.direct_inverse_transform(data, grid)
        inverse_bt_pix_flat = (inverse_bt_pix * mask_bmax).flatten(start_dim=1)

        # Calculate squared difference (MSE) as regularization
        # Per-sample MSE on masked pixels (B,)
        mse = ((inverse_bt_pix_flat - pix_bt_flat) ** 2).mean(dim=1)
        
        # CC "regularization" (B,) and also return mse for logging
        cc_reg = -0.5 * mse / (self.cc_std_bt**2)

        return {"cc_mse": mse, "cc_reg": cc_reg}
    ...
    def kinematic_mask(self, grid):
        """
        Returns a float mask with shape (x, Q2, z, qT, phi) in {0,1}.
        Kinematic cuts:
          - y < 1
          - W^2 > W2_min
          - qT/Q < qTmax(x,Q2,z) from the provided formula
        """
        x  = grid[0]  # (nx,)
        Q2 = grid[1]  # (nQ2,)
        z  = grid[2]  # (nz,)
        ...
        # constants
        M  = torch.tensor(0.938, device=device, dtype=dtype)
        M2 = M * M
        Mh = torch.tensor(0.13957, device=device, dtype=dtype)  # pion mass; adjust if needed
        Mh2 = Mh * Mh
    
        s = torch.tensor(2.0, device=device, dtype=dtype) * torch.tensor(self.Ebeam, device=device, dtype=dtype) * M + M2
        
        # physics x_min = Q2_min / (s - M^2)
        x_min_phys = torch.tensor(self.Q2_min, device=device, dtype=dtype) / (s - M2)  # scalar tensor
        mask_xmin  = (x >= x_min_phys)  
        
        Q = torch.sqrt(Q2)  # (nQ2,)
    
        # --- y < 1 ---
        # y = Q2 / (x (s - M^2))
        y = torch.einsum('Q,x->xQ', Q2, 1.0 / x) / (s - M2)  # (nx, nQ2)
        mask_y = (y < 1.0)
    
        # --- W^2 > W2_min ---
        # W^2 = M^2 + (Q2/x)*(1-x)
        W2 = M2 + torch.einsum('Q,x->xQ', Q2, (1.0 - x) / x)  # (nx, nQ2)
        mask_W2 = (W2 > torch.tensor(self.W2_min, device=device, dtype=dtype))
    
        # --- qT/Q < bound(x,Q2,z) ---
        # Build broadcast grids (x,Q2,z)
        x_xQz  = x[:, None, None]          # (nx,1,1)
        Q2_xQz = Q2[None, :, None]         # (1,nQ2,1)
        Q_xQz  = Q[None, :, None]          # (1,nQ2,1)
        z_xQz  = z[None, None, :]          # (1,1,nz)
    
        rho2 = 1.0 + 4.0 * M2 * x_xQz**2 / Q2_xQz  # (nx,nQ2,nz)
    
        # RHS^2 of your inequality (call it R2). We will clamp it >= 0 to avoid NaNs.
        # termA = 1/(rho^2-1) * [ 1 - (rho^2-1) * M^2/(Q^2 z^2) ]
        denom = (rho2 - 1.0)
    
        termA = (1.0 / denom) * (1.0 - denom * M2 / (Q2_xQz * z_xQz**2))
    
        # termB = (rho^2-1)/(4 rho^2 z^2) * [ ... ]^2
        bracket = (
            (1.0 - x_xQz - z_xQz) / x_xQz
            + Mh2 / Q2_xQz
            - 2.0 * z_xQz / denom
        )
        termB = (denom / (4.0 * rho2 * z_xQz**2)) * (bracket**2)
    
        R2 = termA - termB
        R2_clamped = torch.clamp(R2, min=0.0)
        qTmax_over_Q = torch.sqrt(R2_clamped)  # (nx,nQ2,nz)
        
        # --- additional applicability cut: qT/Q < qT_lim ---
        qTlim = torch.tensor(self.qT_lim, device=device, dtype=dtype)  # scalar
        qTmax_over_Q = torch.minimum(qTmax_over_Q, qTlim)

        # Compare qT/Q against bound
        qT_over_Q = qT[None, None, None, :] / Q_xQz[:, :, :, None]  # (nx,nQ2,nz,nqt)
        mask_qT = (qT_over_Q <= qTmax_over_Q[:, :, :, None])
    
        # Combine (x,Q) masks, then expand over z,q,phi
        mask_xQ = (mask_y & mask_W2 & mask_xmin[:, None])[:, :, None, None]  # (nx,nQ2,1,1)
        mask_xQzq = mask_xQ & mask_qT
        mask = mask_xQzq[:, :, :, :, None].expand(-1, -1, -1, -1, phi.numel())
    ...
    def compute_cross_sections(self, data, grid):
        ...
        # --apply kinematic mask (set unphysical regions to 0)
        mask = self.kinematic_mask(grid).to(sigma.device).to(sigma.dtype)
        sigma = sigma * mask
        
        # --ENSURE THAT SIGMA IS POSITIVE (optional; keep if you still want this behavior)
