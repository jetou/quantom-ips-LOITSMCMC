...
class GANOptimizer(StatefulModule):
    def __init__(
        ...
        cc_weight: float = 0,        # 0 disables CC
        cc_warmup_epochs: int = 0,     # CC off for first N epochs
        cc_freq: int = 1,              # compute CC every N epochs
        ...
        self.cc_warmup_epochs = cc_warmup_epochs
        self.cc_freq = cc_freq
    ...
    def _epoch_metrics(self, epoch, gen_loss, disc_loss, input_mse, xsec_mse, tmd_mse, cc_mse):
        metrics = {
    ...
    def train(self, env, data_parser):
        ...
        cc_mse     = torch.full((n_epochs,), float("nan"))
        cc_mse_std = torch.full((n_epochs,), float("nan"))       
        ...
        def _cc_mse_from_theory(params_for_theory, grid_axes):
            """
            Returns per-sample CC MSE tensor of shape (B,).
            Expects theory.get_loss to return dict with key 'cc_mse' OR a tensor.
            """
            out = env.theory.get_loss(params_for_theory, grid_axes)
        
            if isinstance(out, dict):
                if "cc_mse" in out:
                    return out["cc_mse"]
                # fallback if you kept cc_reg only (cc_reg = -0.5 * mse / std^2)
                if "cc_reg" in out:
                    return (-2.0 * out["cc_reg"])
                raise KeyError(f"theory.get_loss() dict missing 'cc_mse'/'cc_reg'. keys={list(out.keys())}")
        
            # assume tensor already
            return out
        ...
        for epoch in epoch_iter:
            ...
            if self.train_on == "events":
                ...
                for _ in iter_steps:
                    ...
                    # --------------------------
                    # Cycle Consistency regularization (events mode only)
                    # --------------------------
                    do_cc = (
                        (self.cc_weight is not None) and (self.cc_weight > 0.0)
                        and getattr(env.theory, "enable_cc", False)
                        and (epoch >= int(self.cc_warmup_epochs))
                        and ((epoch % int(self.cc_freq)) == 0)
                    )
                    ...
                    if do_cc:
                        ...
                        params_for_theory_cc = ensure_flavor_dim(gen_tmd)

                        cc_mse_batch = _cc_mse_from_theory(params_for_theory_cc, grid_axes)  # (B,)
                        cc_loss = cc_mse_batch.mean()
                        gen_loss = gen_loss + float(self.cc_weight) * cc_loss
            ...
            if do_mse:
                with torch.no_grad():
                    ...
                    if not free_tmd_mode:
                        ...
                        # --------------------------
                        # CC diagnostics (no grad)
                        # --------------------------
                        if getattr(env.theory, "enable_cc", False):
                            # CC currently defined for SIDIS (non-free) path where get_tmd_pdf exists
                            if not free_tmd_mode:
                                cc_mse_eval = _cc_mse_from_theory(gen_tmd_for_theory, grid_axes)  # (B,)
                                cc_mse[epoch]     = cc_mse_eval.mean().detach().cpu()
                                cc_mse_std[epoch] = cc_mse_eval.std(unbiased=False).detach().cpu()
            ...
            metrics = self._epoch_metrics(
                epoch, gen_loss, disc_loss, input_mse, xsec_mse, tmd_mse, cc_mse
            )
            if self.progress_bar:
