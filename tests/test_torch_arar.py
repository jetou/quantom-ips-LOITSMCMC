...
from quantom_ips.gradient_transport.torch_arar import (
    TorchARAR,
)
...
def setup_conv_arar_test():
    gradient_transport = TorchARAR(gradient_sync_mode="conv_arar")
...
@pytest.fixture
def setup_arar_test():
    gradient_transport = TorchARAR(gradient_sync_mode="arar", group_size=20)
    rank = gradient_transport.rank
    yield gradient_transport, rank, False
...
def setup_strong_arar_test():
    gradient_transport = TorchARAR(gradient_sync_mode="strong_arar", group_size=20)
    rank = gradient_transport.rank
    yield gradient_transport, rank, False
...
def setup_rma_arar_test():
    gradient_transport = TorchARAR(gradient_sync_mode="rma_arar", group_size=20)
    rank = gradient_transport.rank
    yield gradient_transport, rank, False


class TestTorchARAR(TestTorchGradientTransport):
